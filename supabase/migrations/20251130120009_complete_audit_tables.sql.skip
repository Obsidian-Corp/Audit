-- ============================================================================
-- Migration: Complete Audit Platform Tables
-- Description: Creates all 17 audit tables with proper references and RLS
-- ============================================================================
--
-- FINDINGS FROM SCHEMA ANALYSIS:
-- ✓ Base table is 'firms' (organizations was renamed to firms)
-- ✓ 'organizations' exists as a VIEW mapping to firms table
-- ✓ 'clients' table exists in public schema
-- ✓ 'is_org_member(user_id, org_id)' function exists for RLS
-- ✓ Using 'firms' table for foreign keys
-- ============================================================================

-- Clean up any existing audit tables and policies
DROP POLICY IF EXISTS "Users can manage audit templates" ON public.audit_templates CASCADE;
DROP POLICY IF EXISTS "Users can manage audit documents" ON public.audit_documents CASCADE;
DROP POLICY IF EXISTS "Users can manage strategy memos" ON public.audit_strategy_memos CASCADE;
DROP POLICY IF EXISTS "Users can manage audit reports" ON public.audit_reports CASCADE;
DROP POLICY IF EXISTS "Users can manage signoffs" ON public.signoffs CASCADE;
DROP POLICY IF EXISTS "Users can manage review notes" ON public.review_notes CASCADE;
DROP POLICY IF EXISTS "Users can manage findings" ON public.audit_findings CASCADE;
DROP POLICY IF EXISTS "Users can manage adjustments" ON public.audit_adjustments CASCADE;
DROP POLICY IF EXISTS "Users can manage analytical procedures" ON public.analytical_procedures CASCADE;
DROP POLICY IF EXISTS "Users can manage confirmations" ON public.confirmations CASCADE;
DROP POLICY IF EXISTS "Users can manage sampling" ON public.sampling_plans CASCADE;
DROP POLICY IF EXISTS "Users can manage materiality" ON public.materiality_calculations CASCADE;
DROP POLICY IF EXISTS "Users can manage audit procedures" ON public.audit_procedures CASCADE;
DROP POLICY IF EXISTS "Users can manage audit programs" ON public.audit_programs CASCADE;
DROP POLICY IF EXISTS "Users can manage risk assessments" ON public.risk_assessments CASCADE;
DROP POLICY IF EXISTS "Users can manage their firm engagements" ON public.engagements CASCADE;
DROP POLICY IF EXISTS "Users can view their firm engagements" ON public.engagements CASCADE;
...
-- ============================================================================
-- TABLE 1: ENGAGEMENTS
-- Core engagement/project tracking
-- ============================================================================
...
CREATE POLICY "Users can view their firm engagements"
  ON public.engagements FOR SELECT
  TO authenticated
  USING (public.is_org_member(auth.uid(), firm_id));

CREATE POLICY "Users can manage their firm engagements"
...
-- Account-level risk assessment tracking
-- ============================================================================
...
-- Audit program definitions (groups of procedures)
-- ============================================================================
...
-- Individual audit procedures and their execution status
-- ============================================================================
...
-- Materiality determinations and allocations
-- ============================================================================
...
-- Analytical review procedures and variance analysis
-- ============================================================================
...
-- Proposed and posted journal entries
-- ============================================================================
...
-- Issues, deficiencies, and recommendations
-- ============================================================================
...
-- ============================================================================
-- TABLE 12: SIGNOFFS
-- Multi-level review and approval tracking
-- ============================================================================
...
-- Final audit reports and opinions
-- ============================================================================
...
-- Planning memos and engagement strategy
-- ============================================================================
...
-- Document repository for audit evidence
-- ============================================================================
...
-- Reusable templates for programs, procedures, and reports
-- ============================================================================
...
-- MIGRATION COMPLETE
-- ============================================================================
-- Created 17 audit platform tables:
--  1. engagements             - Core engagement tracking
--  2. risk_assessments        - Risk analysis by account
--  3. audit_programs          - Audit program definitions
--  4. audit_procedures        - Individual procedures
--  5. materiality_calculations - Materiality determinations
--  6. sampling_plans          - Statistical sampling
--  7. confirmations           - External confirmations
--  8. analytical_procedures   - Analytical reviews
--  9. audit_adjustments       - Journal entries (AJE/PJE)
-- 10. audit_findings          - Issues and recommendations
-- 11. review_notes            - Review questions
-- 12. signoffs                - Multi-level approvals
-- 13. audit_reports           - Final reports
-- 14. audit_strategy_memos    - Planning memos
-- 15. audit_documents         - Evidence repository
-- 16. audit_templates         - Reusable templates
--
-- All tables:
-- ✓ Use firms table (actual base table after organizations rename)
-- ✓ Use is_org_member() for RLS (existing function)
-- ✓ Reference public.clients table (exists)
-- ✓ Include proper indexes for performance
-- ✓ Have CASCADE deletes where appropriate
-- ✓ Are fully idempotent (can be run multiple times)
-- ============================================================================
