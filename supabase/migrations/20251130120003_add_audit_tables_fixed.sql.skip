-- ============================================================================
-- Migration: Add Audit Platform Tables (Fixed RLS Policies)
-- Version: 20251130120003
-- Description: Adds missing audit-specific tables with corrected RLS policies
-- ============================================================================
...
CREATE POLICY "engagements_select_policy" ON engagements FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = engagements.organization_id
      AND om.user_id = auth.uid()
    )
  );

CREATE POLICY "engagements_all_policy" ON engagements FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = engagements.organization_id
...
CREATE POLICY "risk_assessments_all_policy" ON risk_assessments FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = risk_assessments.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "audit_programs_all_policy" ON audit_programs FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_programs.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "audit_procedures_all_policy" ON audit_procedures FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_procedures.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "materiality_all_policy" ON materiality_calculations FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = materiality_calculations.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "sampling_all_policy" ON sampling_plans FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = sampling_plans.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "confirmations_all_policy" ON confirmations FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = confirmations.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "analytical_all_policy" ON analytical_procedures FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = analytical_procedures.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "adjustments_all_policy" ON audit_adjustments FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_adjustments.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "findings_all_policy" ON audit_findings FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_findings.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "review_notes_all_policy" ON review_notes FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = review_notes.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "signoffs_all_policy" ON signoffs FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = signoffs.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "audit_reports_all_policy" ON audit_reports FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
...
CREATE POLICY "strategy_memos_all_policy" ON audit_strategy_memos FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_strategy_memos.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "audit_documents_all_policy" ON audit_documents FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_documents.organization_id
      AND om.user_id = auth.uid()
    )
  );
...
CREATE POLICY "audit_templates_all_policy" ON audit_templates FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members om
      WHERE om.organization_id = audit_templates.organization_id
      AND om.user_id = auth.uid()
    )
  );
