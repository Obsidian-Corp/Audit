-- FIX RLS RECURSION FOR CORE AUDIT TABLES
-- These all have policies that check user_roles, causing infinite loops
...
-- Note: risks table uses organization_id, not firm_id
...
CREATE POLICY "Users can view risks in their firm"
  ...
  FOR SELECT
  TO authenticated
  USING (organization_id = public.user_firm_id(auth.uid()));
...
COMMENT ON FUNCTION public.user_has_role IS 'Helper function to check user role without RLS recursion';
COMMENT ON FUNCTION public.user_firm_id IS 'Helper function to get user firm_id without RLS recursion';
